...
import numpy
import settings
from utils import keyword2unicode_seq, unicode_converter, color_me
import pickle
from stackSearch import stackSearch
import copy

N_TOLERANT_UNK_SIZE=0
...
class keywordManager():
    def __init__(self, keyword_list = None, keyword_file = None):
        ...
        self._tolerant_unk_size = N_TOLERANT_UNK_SIZE
    ...
    def stack_match(self, query_seq, n_tolerant_unmatched_word, threshold_score = 1.0e-5, is_show_all = True):
        ...
        for loc, word in enumerate(query_seq):
            ...
            if wordid != self._unk[0]:
                for docid, wordid_loc, is_last in self._inverted_index[wordid]:
                    stack_monitor.expand(wordid, wordid_loc, docid, is_last, loc)
            else:
        ...
        for mm in matched.values():
            for loc in mm[2]:
                ...
                epos = loc[1]
                if ht_seq == '':
                    ht_seq = ''.join(query_seq[:spos])
                seg = ''.join(query_seq[spos:epos+1])
                seg = color_me(seg, color_text='red')
                ht_seq += seg
        '''
        keyword_loc = {}
        for mm in matched.values():
            for spos, epos in mm[2]:
                if spos not in keyword_loc:
                    keyword_loc[spos] = epos
                else:
                    if epos > keyword_loc[spos]:
                        keyword_loc[spos] = epos
        
        #print(''.join(query_seq))
        #print('-----------')
        sloc = sorted(keyword_loc)
        pos0 = 0
        ht_seq = ''
        for st in sloc:
            en = keyword_loc[st]
            ht_seq += ''.join(query_seq[pos0:st])
            seg = ''.join(query_seq[st:en+1])
            seg = color_me(seg, color_text='blue', color_bk='white')
            ht_seq += seg
            pos0 = en + 1
            if (st, en) == (sloc[-1], keyword_loc[sloc[-1]]):
                ht_seq += ''.join(query_seq[pos0:len(query_seq)])
        print(ht_seq)
...
if __name__ == '__main__':
    ...
    keyword_index.index('words.list') #settings.TEST_KEYWORD)
    ...
    #keyword_index.save_index('index.json', readable = False)
    matched_keywords, matched_score = keyword_index.query(['普选'], is_show_all = True)
